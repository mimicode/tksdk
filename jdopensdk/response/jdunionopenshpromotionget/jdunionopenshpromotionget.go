package jdunionopenshpromotionget
...
// Response jd.union.open.sh.promotion.get 深海投流账户授权模式获取推广链接接口【申请】
type Response struct {
	response.TopResponse
	Responce Responce `json:"jd_union_open_sh_promotion_get_responce"`
}
...
type Responce struct {
	...
	GetResult    GetCodeByTaskIdResult
}

// GetCodeByTaskIdResult 返回结果
type GetCodeByTaskIdResult struct {
	Code    int64               `json:"code"`
	Data    GetCodeByTaskIdResp `json:"data"`
	Message string              `json:"message"`
}

// GetCodeByTaskIdResp 数据明细
type GetCodeByTaskIdResp struct {
