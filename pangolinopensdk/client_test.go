...
import (
	...
	"github.com/mimicode/tksdk/pangolinopensdk/response/livelink"
...
var (
	pangolinAppKey    string
	pangolinAppSecret string
)
	...
	if _, err := os.Stat("../dev_env.json"); err == nil {
		if bytes, err := ioutil.ReadFile("../dev_env.json"); err == nil {
			...
			if err = json.Unmarshal(bytes, &data); err == nil {
				pangolinAppKey = data.Pangolin.AppKey
				pangolinAppSecret = data.Pangolin.AppSecret
			}
		}
	}
}
...
func GetClient() *TopClient {
	client := &TopClient{}
	client.Init(pangolinAppKey, pangolinAppSecret, "")
	return client
}
...
func TestProductDetail(t *testing.T) {
	...
	err := client.Execute(searchReq.GetApiName(), searchReq, &searchResp)
	if err != nil {
	...
	err = client.Execute(detailReq.GetApiName(), detailReq, &detailResp)
	if err != nil {
...
func TestProductLink(t *testing.T) {
	...
	err = client.Execute(linkReq.GetApiName(), linkReq, &linkResp)
	if err != nil {
